...
{
  [SerializeField] BoatData boatData;
    ...
    private void Update()
    {
        Collider[] colliders = Physics.OverlapSphere(transform.position, boatData.neighborhoodRadius);
    ...
    private Vector3 ComputeAcceleration(IEnumerable<BoatAutoPilot> boats)
    {
        ...
        acceleration += ComputeAlignment(boats) * boatData.alignmentAmount;
        acceleration += ComputeSeparation(boats) * boatData.separationAmount;
        acceleration += ComputeCohesion(boats) * boatData.cohesionAmount;
    ...
    private void UpdateVelocity(Vector3 acceleration)
    {
        ...
        velocity = LimitMagnitude(velocity, boatData.maxSpeed);
    }
    ...
    private void UpdateRotation(Vector3 velocity)
    {
        ...
        transform.forward = Vector3.RotateTowards(transform.forward, velocity, Time.deltaTime * boatData.steeringSpeed, float.MaxValue);
    }
    ...
    private Vector3 ComputeAlignment(IEnumerable<BoatAutoPilot> boats)
    {
        ...
        var steer = Steer(velocity.normalized * boatData.maxSpeed);
        return steer;
    }
    ...
    private Vector3 ComputeCohesion(IEnumerable<BoatAutoPilot> boats)
    {
        ...
        var steer = Steer(direction.normalized * boatData.maxSpeed);
        return steer;
    }
    ...
    private Vector3 ComputeSeparation(IEnumerable<BoatAutoPilot> boats)
    {
        ...
        boats = boats.Where(boat => Vector3.Distance(transform.position, boat.transform.position) <= boatData.separationRadius);
    ...
    private Vector3 Steer(Vector3 desiredVelocity)
    {
        ...
        steer = LimitMagnitude(steer, boatData.maxForce);
        return steer;
    }
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        Gizmos.DrawWireSphere(transform.position, boatData.neighborhoodRadius);

        // Separation radius.
        Gizmos.color = Color.salmon;
        Gizmos.DrawWireSphere(transform.position, boatData.separationRadius);
    }
}